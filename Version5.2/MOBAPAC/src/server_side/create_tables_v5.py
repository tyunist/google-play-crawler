...
class Comments(Base, BaseMixin):
    ...
    comment_id = Column(BIGINT(UNSIGNED=True), nullable=False, primary_key=True)
    app_index = Column(INTEGER(UNSIGNED=True), ForeignKey('id.app_index', ondelete='cascade',onupdate='cascade'), nullable=False)
...
# #------------------------------------------------------------------------------ 
# #The following classes are for testing the efficiency of searching apps by keywords.
...
class Report(Base, BaseMixin):
    """ Report class that holds information of apps, comments on daily basis """
    __tablename__ = 'report'

    report_id = Column(INTEGER(UNSIGNED=True), nullable=False, primary_key=True)
    app_number = Column(INTEGER(UNSIGNED=True),nullable=False, default=0)
    died_app_number = Column(INTEGER(UNSIGNED=True),nullable=False, default=0)
    comment_number = Column(INTEGER(UNSIGNED=True), nullable=False, default=0)
    developer_number = Column(INTEGER(UNSIGNED=True), nullable=False, default=0)
    category_number = Column(INTEGER(UNSIGNED=True), nullable=False, default=0)
    
    def __init__(self, app_number, died_app_number,comment_number, developer_number, category_number):
        self.app_number = app_number
        self.died_app_number = died_app_number
    ...
    def __repr__(self):
        return "<Report('%s','%s', '%s','%s','%s','%s','%s','%s')>" % (self.report_id, self.created_at, self.updated_at, self.app_number, self.died_app_number, self.comment_number, self.developer_number, self.category_number)
