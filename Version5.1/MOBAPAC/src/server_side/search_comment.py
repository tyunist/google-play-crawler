...
import time
import datetime.datetime.utcnow as now

def write_log(list):
    with open("re_search_comment.csv", "a+b") as f:
...
write_log([now(),"Searching for new comments"])
...
for pack in app_tuple:
    try:
        ...
        write_log([now(),"SSL Error occur", pack])
        time.sleep(10)
        #Log in a gain
        api = GooglePlayAPI(ANDROID_ID)
        api.login(GOOGLE_LOGIN, GOOGLE_PASSWORD, AUTH_TOKEN)
...
write_log([now(),"Fetching app_index"])
...
write_log([now(),"Inserting new comments of new apps"])
N_comment_new_app = 0
N_comment_old_app = 0
insert_new_comment__at = datetime.datetime.utcnow()
for i in new_set_app_index:
    if session.query(Comments.app_index).filter(Comments.app_index==i).count() is None:
        ...
        write_log([now(),"Finish inserting new comments of new apps"])
        ...
        insert_old_comment__at = datetime.datetime.utcnow()
        write_log([now(),"Inserting new comments of old apps"])
...
write_log([now(),"Finish inserting new comments of old apps"])



write_log(["*************Summarize!**********************"])
write_log(["No of new comments of new apps:", N_comment_new_app])
write_log(["No of new comments of old apps:", N_comment_old_app])
write_log(["Searching for new comments at:!", search_comment_at])
write_log(["Inserting new comments of new apps at!", insert_new_comment__at])
write_log(["Inserting new comments of old apps at!", insert_old_comment__at])
write_log(["End process at:",now()])
write_log(["*************END*****************************"]) 
...
print "Inserting new comments of new apps at!", insert_new_comment__at
