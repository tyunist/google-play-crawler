...
def write_log(list):
    with open("re_search_comment_v4.csv", "a+b") as f:
...
write_log([datetime.datetime.utcnow(),'Start search_comment process!'])
...
# write_log([datetime.datetime.utcnow(),'Fetching IDs from id table!'])
# app_tuple = session.query(Id).all()
# write_log([datetime.datetime.utcnow(),"Number of apps:", session.query(Id.app_index).count()])
...
write_log([datetime.datetime.utcnow(),"Searching for new comments"])
current_app_index = 0
while(True):
    #Here, we have not consider the case of died app whose no information detail is found but sometimes, we could find its comments (dont know why ^^). 
    AppTemp = session.query(Id).filter(Id.app_index> current_app_index).first()
    if AppTemp is None: #End of the id table
        write_log([datetime.datetime.utcnow(),"End of id table!"])
        break
    if session.query(func.count(Id_died.app_index)).filter(Id_died.app_index==AppTemp.app_index).scalar() != 0: # If app is died, skip it from searching comments
        ...
        write_log([datetime.datetime.utcnow(),"Current app_index:%d, app_id: %s. App is died!"%(AppTemp.app_index,AppTemp.app_id)])
        continue
    current_app_index = AppTemp.app_index
    N_app_total +=1
    write_log([datetime.datetime.utcnow(),"Current app_index:%d, app_id: %s. Searching comments"%(AppTemp.app_index,AppTemp.app_id)])
    try:
        comment_list_of_dict = get_comment_dict(AppTemp.app_id, api)
    except ConnectionError as e:
        write_log([datetime.datetime.utcnow(),e])
        time.sleep(60)
        #Log in a gain
        log_in()
        continue
#     except KeyError as e:
#         write_log([datetime.datetime.utcnow(),"App_id: %s is wrong! "%AppTemp.app_id])
#         continue
    except SSLError as e:
        print "SSL Error occur!", e
        write_log([datetime.datetime.utcnow(),"SSL Error occur, current app_id:%s"%AppTemp.app_id])
        time.sleep(60)
        #Log in a gain
        log_in()
        continue
        ...
        N_comment_updated_each = 0
        write_log([datetime.datetime.utcnow(),"Number of searched comments:%d"%N_comment_searched_each])
    ...
    #If app_ip has not existed already in comments list, inserts the whole of its comments:
        if session.query(func.count(distinct(Comments.app_index))).filter(Comments.app_index== AppTemp.app_index).scalar()==0:
            write_log([datetime.datetime.utcnow(),AppTemp.app_index,"is new, preparing to insert. No of comments:", N_comment_searched_each])
            ...
            write_log([datetime.datetime.utcnow(),"Insert successfully! %d comments"%N_comment_inserted_each]) 
            continue
     
    #If app_ip existed already in comments list, we have to check whether a comment is new or not before inserting     
        else: 
            write_log([datetime.datetime.utcnow(),AppTemp.app_index,"is old, preparing to insert. No of comments:", N_comment_searched_each])
    ...
    #                 write_log([datetime.datetime.utcnow(),"Comment is old. Do nothing"])
                    continue
    ...
    #Case 4:        
                if session.query(Comments.id).filter(and_(
                    ...
                    write_log([datetime.datetime.utcnow(),"Comment_by_user:",comment_dict.get('commentId',''.decode('utf8')), " is updated!"])
                    N_comment_inserted_each -=1
                    N_comment_updated_each +=1
                    session.flush()
                    write_log([datetime.datetime.utcnow(),"Comment has been updated by user. Updated!"])
                    continue
    #The remaining cases
            #Just insert all comments
                ...
                session.add(comments)
                session.flush()
    #             write_log([datetime.datetime.utcnow(),"Comment is new, inserted!"])
            session.commit()
            N_comment_inserted_total += N_comment_inserted_each
            N_comment_updated_total += N_comment_updated_each
            write_log([datetime.datetime.utcnow(),"Inserted: %d comments"%N_comment_inserted_each])
            write_log([datetime.datetime.utcnow(),"Updated: %d comments"%N_comment_updated_each])


write_log([datetime.datetime.utcnow(),"Finish inserting comments"])

write_log(["*************Summarize!**********************"])
write_log(["No of apps in total:", N_app_total])
write_log(["No of searched comments in total:", N_comment_searched_total])
write_log(["No of N_comment_inserted_total:", N_comment_inserted_total])
write_log(["No of N_comment_updated_total:", N_comment_updated_total])
write_log(["No of new comments of new apps", N_comment_inserted_new_app])
write_log(["No of new comments of old apps",(N_comment_inserted_total- N_comment_inserted_new_app)])
write_log(["Start process at:", started_at])
