...
import datetime, time, csv, requests
...
def log_in():
    try:
        ...
        write_log([datetime.datetime.utcnow(),"Connection Error while trying to log in Google Store"])
        raise  

def write_log(list):
    with open("re_update_infor.csv", "a+b") as f:
...
write_log([datetime.datetime.utcnow(),"Start searching app information details"])
...
for AppTemp in session.query(Id).all():   
    ...
    try:
        ...
        write_log([datetime.datetime.utcnow(),"Connection Error while trying to search information details. Roll back today's changes and stop program"])
        session.rollback()
        sys.exit(1)  
    except SSLError:
        print "SSL Error occur!"
        write_log([datetime.datetime.utcnow(),"SSL Error occur with current app_id:",app_id])
...
#             session.delete(AppTemp)
                ...
                session2.rollback()
                write_log([datetime.datetime.utcnow(),"%s : Duplicate entry with %s!"%(e,app_id)])
            print "Died app!", app_id
            write_log([datetime.datetime.utcnow(),"%s is coppied to iddied table!"%app_id])
            N_died_infor +=1
            continue
        ...
        write_log([datetime.datetime.utcnow(),"%s is preparing to insert information details!"%app_id])
...
#------------------------------------------------------------------------------ 
        ...
        try:
            ...
            write_log([datetime.datetime.utcnow(), "KeyError with google_pluses!", k])
            pass
        ...
        try:
            ...
            updated_infor.app_avar_rating = infor_dict['aggregateRating'].get('starRating', 0)    # Because the result given by Google is float
            updated_infor.app_comments = infor_dict['aggregateRating'].get('commentCount', 0)
        except KeyError as k:
            write_log([datetime.datetime.utcnow(), "KeyError with aggregateRatings!", k])
            pass
...
#         category_name = ''.decode('utf8')
        try:
            ...
            write_log([datetime.datetime.utcnow(), "KeyError with appDetails", k])
            print "Issue here 1"
            pass
        ...
        try:
            ...
            write_log([datetime.datetime.utcnow(), "KeyError with offer!", k])
            pass
        ...
        try:
            ...
            write_log([datetime.datetime.utcnow(), "KeyError with availablity!", k])
            pass
...
#------------------------------------------------------------------------------ 
        if session.query(func.count(Types.type_index)).filter(Types.type_name==type_name).scalar()==0: #Not exist?
            ...
            session.flush()
            print "New type"
            write_log([datetime.datetime.utcnow(), "New type :!", type_name])
        ...
        for permission_name in permission_list:
            if session.query(func.count(Permissions.permission_index)).filter(Permissions.permission_name==permission_name).scalar()==0:
                ...
                write_log([datetime.datetime.utcnow(), "New permission_name :!", permission_name])
        ...
        if session.query(func.count(Developers.developer_index)).filter(and_(Developers.developer_name==developer_name,
            ...
            write_log([datetime.datetime.utcnow(), "New developer_name:!", developer_name])
        ...
        if session.query(func.count(Categories.category_index)).filter(Categories.category_name==category_name).scalar()==0:
            ...
            write_log([datetime.datetime.utcnow(), "New category_name:!", category_name])
        ...
        if N_updated_infor== 10:
            write_log(["Time to finish updating information for %d apps:"%N_updated_infor, (datetime.datetime.utcnow() - started_at)])
...
write_log(["*************Summarize!**********************"])
write_log(["Number of row inserted into table updated_infor:",N_updated_infor])
write_log(["Number of row removed from id table (died app_id):",N_died_infor])
write_log(["Start searching app information details at:", started_at])
write_log(["End process at:",datetime.datetime.utcnow()])
write_log(["Total time:",(datetime.datetime.utcnow()-started_at)])
write_log(["*************END*****************************"]) 
...
#         print "Test  types:"
#         print updated_infor.app_title, type(updated_infor.app_title) 
#         print  updated_infor.app_description, type(updated_infor.app_description) 
#         print updated_infor.app_google_pluses, type(updated_infor.app_google_pluses)
#         print updated_infor.app_one_ratings , type(updated_infor.app_one_ratings ) 
#         print updated_infor.app_two_ratings, type(updated_infor.app_two_ratings) 
#         print updated_infor.app_ratings, type(updated_infor.app_ratings) 
#         print updated_infor.app_avar_rating, type(updated_infor.app_avar_rating) 
#         print updated_infor.app_comments, type(updated_infor.app_comments) 
#         print updated_infor.app_size, type(updated_infor.app_size) 
#         print  updated_infor.app_released_at, type( updated_infor.app_released_at) 
#         print updated_infor.app_version_number, type(updated_infor.app_version_number) 
#         print updated_infor.app_what_new, type(updated_infor.app_what_new) 
#         print updated_infor.app_downloads, type(updated_infor.app_downloads) 
#         print updated_infor.app_content_rating, type(updated_infor.app_content_rating) 
#         print updated_infor.app_detail_url, type(updated_infor.app_detail_url) 
#         print updated_infor.app_version_string, type(updated_infor.app_version_string) 
#         print updated_infor.app_currency, type(updated_infor.app_currency) 
#         print updated_infor.app_price, type(updated_infor.app_price) 
#         print  updated_infor.restriction_channel_id, type( updated_infor.restriction_channel_id) 
#         print updated_infor.restriction_device, type(updated_infor.restriction_device) 
#         print updated_infor.restriction_android_id, type(updated_infor.restriction_android_id) 
#         print "End of test types" 
