'''
Edited on Oct 23, 2013
@attention: This module is edited from open-source Google_api to connect to Google store via proxy and send request to get data
NOTE: only use either Newgoogleplay or googleplay module.
@author: tynguyen
'''
__author__="Ty"
__date__ ="$Oct 21, 2013 5:09:43 PM$"
...
class GooglePlayAPI(object):
    ...
    def __init__(self, androidId=None, lang=None, debug=False,pip=None): # you must use a device-associated androidId value
        self.preFetch = {}
        if androidId == None:
            androidId = __init__.ANDROID_ID
        if lang == None:
            lang = __init__.LANG
        if pip == None:
            pip = __init__.PIP
    ...
    def setAuthSubToken(self, authSubToken):
        ...
        # put your auth token in __init__.py to avoid multiple login requests
        if self.debug:
...
#            response= resp.read()
    ...
    def executeRequestApi2(self, path, datapost=None, post_content_type="application/x-www-form-urlencoded; charset=UTF-8", pip = None):
...
#            opener.addheaders = [headers]
            urllib2.install_opener(opener)
            req=urllib2.Request(url=url,data=datapost,headers=headers,unverifiable=False)
            response=urllib2.urlopen(req)
#            response = resp.read()
...
#            print "headers:", headers
        ...
        data = gzipper.read()
        '''
        message = googleplay_pb2.ResponseWrapper.MergeFromString(data)
    ...
    def search(self, query, nb_results=None, offset=None, pip = None):
        """Search for apps."""
     ...
     #   path +="&hl=%s" %self.lang #
        ...
        message = self.executeRequestApi2(path,pip = pip)
        return message.payload.searchResponse

    def details(self, packageName,pip = None):
    ...
    def bulkDetails(self, packageNames,pip = None):
        ...
        message = self.executeRequestApi2(path, data, "application/x-protobuf",pip)
    ...
    def list(self, cat, ctr=None, nb_results=None, offset=None, pip=None):
        """List apps.
       ...
       # path +="&hl=%s" %self.lang # Set locale
        
        message = self.executeRequestApi2(path,pip=pip)
        return message.payload.listResponse
    
    def reviews(self, packageName, filterByDevice=False, sort = 2, nb_results=None, offset=None,pip = None):
        """Browse reviews.
    ...
    def reviewById(self, packageName, filterByDevice=False, id = "",pip = None):
        """Browse reviews.
        packageName is the app unique ID.
        If filterByDevice is True, return only reviews for your device."""
        print id
        path = "rev?doc=%s&id=%s&n=1" % (requests.utils.quote(packageName),id)
