...
@attention: This module search comments for each app_id stored in id table which are then inserted into comments table
        ...
        3. Existed app_id     Existed given_by_user   New app_version                              -> Insert
        4. Existed app_id     Existed given_by_user   Existed app_version     New comment_given_at  -> Update
        5. Existed app_id     Existed given_by_user   Existed app_version     Existed comment_given_at  -> DO NO THING
@author: tynguyen
'''
...
fetch_id_at = datetime.datetime.now()
...
search_comment_at = datetime.datetime.now()
...
fetch_index_at = datetime.datetime.now()
...
N_comment_new_app = 0
N_comment_old_app = 0
insert_comment__at = datetime.datetime.now()
for i in new_set_app_index:
    if session.query(Comments).filter(Comments.app_index==i).count() is None:
        print "i:", i ,"type:",type(i)
        comment_list_of_dict = total_dict[i]
        for comment_dict in comment_list_of_dict:
            comments = Comments(
                           app_index = i, app_version = comment_dict.get('documentVersion',''.decode('utf8')), 
                           ...
                           device_name = comment_dict.get('deviceName','')
                           )
...
#Case 5:
            if session.query(Comments).filter(and_(
                               Comments.app_index == i,
                               Comments.app_version == comment_dict.get('documentVersion',''.decode('utf8')), 
...
#Case 4:        
            comments =  session.query(Comments).filter(and_(
                               Comments.app_index == i,
                               Comments.app_version == comment_dict.get('documentVersion',''.decode('utf8')), 
...
print "End process at:", datetime.datetime.now()
session.close()
